...
export function History({ items }: { items: HistoryItem[] }) {
  return (
    <section aria-label="History" className="mt-4">
      ...
      {items.map((it) => (
        <div
          key={`${it.title}-${it.airedAt}`}
          className="flex items-center gap-3 border-t border-[color:var(--border)] py-2 opacity-70"
        >
          <div
            className="size-8 shrink-0 rounded-md"
            style={{ background: "linear-gradient(135deg,#33353f,#1c1e25)" }}
          />
