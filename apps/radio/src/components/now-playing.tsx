    ...
    <section aria-label="Now playing">
      ...
      {!np ? (
        <div className="mt-2 flex gap-4">
          ...
          <div className="flex-1 space-y-2 pt-1"><Skeleton className="h-5 w-2/3" /><Skeleton className="h-3 w-1/3" /></div>
        </div>
      ) : (
        <div className="mt-1.5 flex items-start gap-4">
          <div
            className="size-[76px] shrink-0 rounded-lg"
            style={{ background: np.thumbnailUrl ? `center/cover url(${np.thumbnailUrl})` : "linear-gradient(135deg,#3a3d49,#20222a)" }}
          />
          <div className="min-w-0 flex-1">
            <div className="truncate text-lg font-semibold">{np.title}</div>
            {np.artist && <div className="text-sm text-[color:var(--muted-foreground)]">{np.artist}</div>}
            {np.dedication && (
              <div
                className="mt-2 rounded border-l-2 px-2.5 py-1.5 text-xs"
                style={{ color: "var(--radio-amber-soft)", borderColor: "var(--radio-amber)", background: "color-mix(in srgb, var(--radio-amber) 8%, transparent)" }}
              >
                {np.dedication}
              </div>
            )}
