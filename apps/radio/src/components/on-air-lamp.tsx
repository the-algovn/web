...
export function OnAirLamp({ status }: { status: StationStatus }) {
  ...
  const color = status === "off-air" ? "var(--muted-foreground)" : "var(--radio-air)"
  return (
    <span
      className="inline-flex items-center gap-2 rounded-full border px-2.5 py-1 font-mono text-[11px] tracking-[0.12em]"
      style={{ borderColor: "color-mix(in srgb, var(--radio-air) 35%, transparent)", color }}
    >
      <span
        aria-hidden
        ...
        style={{
          ...
          boxShadow: status === "off-air" ? "none" : `0 0 8px 2px ${dim ? "color-mix(in srgb, var(--radio-air) 50%, transparent)" : "var(--radio-air)"}`,
        }}
      />
      {LABEL[status]}
    </span>
