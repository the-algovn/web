...
describe("parseNowPlaying", () => {
  it("accepts a well-formed track", () => {
    const np = parseNowPlaying({
      kind: "track", title: "Em Của Ngày Hôm Qua", artist: "Sơn Tùng M-TP",
      startedAt: "2026-07-20T16:41:00Z", durationSeconds: 258, listeners: 17,
    })
...
describe("parseQueue", () => {
  it("maps items and never surfaces a recipient", () => {
    const q = parseQueue([{ title: "Nơi Này Có Anh", hasDedication: true, recipient: "Ngọc" }])
