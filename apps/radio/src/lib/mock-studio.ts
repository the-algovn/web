...
const SCHEDULE: ScheduleItem[] = [
  { kind: "track", title: "Em Của Ngày Hôm Qua", artist: "Sơn Tùng M-TP", durationSeconds: 258, dedication: "🌙 Gửi Ngọc — chúc ngủ ngon nha", hasDedication: true },
  { kind: "dj", title: "Tiểu Dương Dương tâm sự", durationSeconds: 42 },
  { kind: "track", title: "Nơi Này Có Anh", artist: "Sơn Tùng M-TP", durationSeconds: 269, hasDedication: true },
  { kind: "track", title: "Chạy Ngay Đi", artist: "Sơn Tùng M-TP", durationSeconds: 248 },
  { kind: "track", title: "Lạc Trôi", artist: "Sơn Tùng M-TP", durationSeconds: 234, dedication: "Tặng cả nhà đang thức khuya 💛", hasDedication: true },
  { kind: "jingle", title: "Tần Số 42 — nhận diện đài", durationSeconds: 12 },
  { kind: "track", title: "Muộn Rồi Mà Sao Còn", artist: "Sơn Tùng M-TP", durationSeconds: 271 },
]
...
export class MockStudio implements RadioClient {
  ...
  private position(): { index: number; elapsed: number; itemStartMs: number } {
    ...
    for (; index < SCHEDULE.length; index++) {
      ...
      rem -= SCHEDULE[index]!.durationSeconds
    }
    const before = SCHEDULE.slice(0, index).reduce((s, i) => s + i.durationSeconds, 0)
  ...
  private queueFrom(index: number): QueueItem[] {
    return [1, 2, 3].map(offset => {
      const item = SCHEDULE[(index + offset) % SCHEDULE.length]!
      const q: QueueItem = { title: item.title, hasDedication: item.hasDedication === true }
  ...
  async getHistory(): Promise<HistoryItem[]> {
    ...
    return [1, 2, 3, 4].map(back => {
      const item = SCHEDULE[(index - back + SCHEDULE.length * 10) % SCHEDULE.length]!
      const airedAt = new Date(itemStartMs - back * item.durationSeconds * 1000).toISOString()
  ...
  private tick(): void {
    ...
    this.npSubs.forEach(cb => cb(np))
    this.queueSubs.forEach(cb => cb(q))
  }
