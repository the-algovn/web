  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-1">
        {badge && (
          <div className="text-primary text-[0.65rem] font-bold tracking-widest uppercase">
            {badge}
          </div>
        )}
        <div
        ...
        <div
          className={
            ...
            (unlocked ? "text-foreground italic" : "text-muted-foreground")
          }
        >
      ...
      <ul className="grid gap-4 sm:grid-cols-2">
        {visible.map((entry) => (
          <AchievementCard
            key={entry.id}
            entry={entry}
            variant={variantOf(entry, entries.indexOf(entry))}
          />
        ))}
      </ul>
    </Section>
